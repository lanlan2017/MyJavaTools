...
public class PdfBookmarkOcrStrCorrector {
    ...
    public String correctBookmark111(String pdfBookmarkOcrStr) {
        Pattern pattern = Pattern.compile(RegexEnum.PdfShuQianRegex.toString());
        ...
        if (matcher.matches()) {
            ...
            // 在两个数之间的位置都加上点号
            group1 = group1.replaceAll(RegexEnum.positionBetweenTwoNumbers.toString(), ".");
            group2 = matcher.group(2);
            // 删除前面有中文的空格.
            // final String SpacesAfterChinese = "((?<=[\\u4e00-\\u9fa5]))[ ]";
            group2 = group2.replaceAll(RegexEnum.SpacesAfterChinese.toString(), "$1");
