...
public class MyTree {
    ...
    public static void main(String args[]) {
        new MyTree().mytree(args);
    }

    public void mytree(String[] args) {
     ...
     * @param dir  目录的File对象.
     */
    private void oneArgs(String[] args, File dir) {
        String arg1 = args[0];
        switch (arg1) {
            case "f":
                // mytreeF(dir);
            ...
            default:
                ...
                System.out.println("java:打印目录和文件,只保留Java项目必要的目录,忽略IDE创建的目录,以及.git目录");
    ...
    public void mytreeDir(String path){
        // 保存根目录,后续要用到
        rootPath = path;
        File dir = new File(path);
            ...
            for (int i = 0; i < fileList.length; i++) {
                ...
                if (i + 1 < fileList.length) {
                    if (fileList[i].isDirectory()) {
                        System.out.println("├─" + fileList[i].getName() + File.separator);
                    } else {
     ...
     */
    public void printTreeFileAndDir(File dir, FilenameFilter fileNameFilter) {
        // 打印根目录
        System.out.println(dir.getAbsolutePath());
    ...
    private static void printTreeFileAndDir(File dir, FilenameFilter fileNameFilter, String prefix, int deep) {
        ...
        for (int i = 0; i < childs.length; i++) {
            ...
            if (childs[i].isDirectory()) {
                System.out.println(thisPrefix + childs[i].getName() + File.separator);
                printTreeFileAndDir(childs[i], fileNameFilter, nextPrefix, deep + 1);
            } else
